...
class FuzzyPathPlanner(Node):
    ...
    def plan_callback(self, msg: Path):
        ...
        if not self.ended_goal:
            # Already following a path → ignore until finished
            return
    ...
    def fuzzy_planner(self):
        ...
        #self.get_logger().info(f"F_total→ F_total={np.linalg.norm(F_total)} ")
    ...
    def odom_callback(self, msg: Odometry):
        ...
        #self.get_logger().info(f"/rtabmap/odom received → position=({x:.2f}, {y:.2f}))")
