...
const float GY_F1RO_SCALE = 131.0;     // ±250°/s

// Raw sensor variables
int16_t ax_F1, ay_F1, az_F1, gx_F1, gy_F1, gz_F1;

// Offset variables
float ax_F1_offset = 0, ay_F1_offset = 0, az_F1_offset = 0;
float gx_F1_offset = 0, gy_F1_offset = 0, gz_F1_offset = 0;

//temporary for serial printing
float ax_F1_print=0.0,ay_F1_print=0.0,az_F1_print=0.0,gx_F1_print=0.0,gy_F1_print=0.0,gz_F1_print=0.0;

// Kalman filter states for each gy_F1ro ax_F1is
struct KalmanGy_F1ro {
  float rate = 0.0;
  float bias = 0.0;
  float P[2][2] = {{1, 0}, {0, 1}};
} kalX, kalY, kalZ;
...
//desired speed
float desired_linear_speed_F1 = 0.0;
float desired_angular_speed_F1 = 0.0;
...
void setup() {
  ...
  server.on("/v_desired_F1",HTTP_GET, handle_v_desired_F1);
  server.on("/status_F1",HTTP_GET, handleStatus_F1);
  server.on("/set_pid",HTTP_GET, handlePIDParams);
  server.on("/set_pwm_F1",HTTP_GET, handlePwm_F1);
  server.on("/imu_F1",HTTP_GET,handle_imu_F1); 
  server.begin();
}

void loop() {

  mpu.getMotion6(&ax_F1, &ay_F1, &az_F1, &gx_F1, &gy_F1, &gz_F1);
  ...
  float ax_F1_corr = (ax_F1 - ax_F1_offset) / ACCEL_SCALE;
  float ay_F1_corr = (ay_F1 - ay_F1_offset) / ACCEL_SCALE;
  float az_F1_corr = (az_F1 - az_F1_offset) / ACCEL_SCALE;

  // Raw gy_F1ro rates in °/s after calibration
  float gx_F1_corr = (gx_F1 - gx_F1_offset) / GY_F1RO_SCALE;
  float gy_F1_corr = (gy_F1 - gy_F1_offset) / GY_F1RO_SCALE;
  float gz_F1_corr = (gz_F1 - gz_F1_offset) / GY_F1RO_SCALE;

  // Kalman filtered gy_F1ro
  float filt_gx_F1 = kalman1D(gx_F1_corr, dt, kalX);
  float filt_gy_F1 = kalman1D(gy_F1_corr, dt, kalY);
  float filt_gz_F1 = kalman1D(gz_F1_corr, dt, kalZ);

  float temp_ax_F1 = ax_F1_corr;
  float temp_ay_F1 = ay_F1_corr;
  ax_F1_corr = temp_ay_F1;
  ay_F1_corr = temp_ax_F1;

  float temp_gx_F1 = filt_gx_F1;
  float temp_gy_F1 = filt_gy_F1;
  filt_gx_F1 = temp_gy_F1;
  filt_gy_F1 = temp_gx_F1;

  ax_F1_print=ax_F1_corr;
  ay_F1_print=ay_F1_corr;
  az_F1_print=az_F1_corr;
  gx_F1_print=filt_gx_F1;
  gy_F1_print=filt_gy_F1;
  gz_F1_print=filt_gz_F1;
...
void pid_control_motor() {
  ...
  if (now - last_pid_time >= 25) {  // Run at 40 Hz note that in calculating integral terms we use *0.1/4
    ...
    // Clamp output between 0 and 0.75 (max_F1 duty)
...
void calibrateMPU() {
  long ax_F1_sum = 0, ay_F1_sum = 0, az_F1_sum = 0;
  long gx_F1_sum = 0, gy_F1_sum = 0, gz_F1_sum = 0;

  for (int i = 0; i < 20000; i++) {
    mpu.getMotion6(&ax_F1, &ay_F1, &az_F1, &gx_F1, &gy_F1, &gz_F1);
    ax_F1_sum += ax_F1; ay_F1_sum += ay_F1; az_F1_sum += az_F1;
    gx_F1_sum += gx_F1; gy_F1_sum += gy_F1; gz_F1_sum += gz_F1;
    delay(5);
  }

  ax_F1_offset = ax_F1_sum / 20000.0;
  ay_F1_offset = ay_F1_sum / 20000.0;
  az_F1_offset = (az_F1_sum / 20000.0) - ACCEL_SCALE; // Should be 1g
  gx_F1_offset = gx_F1_sum / 20000.0;
  gy_F1_offset = gy_F1_sum / 20000.0;
  gz_F1_offset = gz_F1_sum / 20000.0;
}

float kalman1D(float newRate, float dt, KalmanGy_F1ro &kal) {
  // Predict
...
//server handlers
void handle_v_desired_F1(){
  if (server.hasArg("desired_linear_velocity_F1")){
    desired_linear_speed_F1=server.arg("desired_linear_velocity_F1").toFloat();
  }
  if (server.hasArg("desired_angular_velocity_F1")){
    desired_angular_speed_F1=server.arg("desired_angular_velocity_F1").toFloat();
  }
  desired_speed_motor1=desired_linear_speed_F1+(desired_angular_speed_F1*wheel_base)/2.0;
  desired_speed_motor2=desired_linear_speed_F1-(desired_angular_speed_F1*wheel_base)/2.0;
...
void handlePwm_F1() {
  if (server.hasArg("initial_pr_F1")) {initial_pr = server.arg("initial_pr_F1").toFloat();}
  if (server.hasArg("initial_pl_F1")) {initial_pl = server.arg("initial_pl_F1").toFloat();}
  server.send(200, "text/plain", "PID parameters updated");
}

void handle_imu_F1(){
  String json = "{";
  json += "\"ax_F1\":" + String(ax_F1_print, 3) + ",";
  json += "\"ay_F1\":" + String(ay_F1_print, 3) + ",";
  json += "\"az_F1\":" + String(az_F1_print, 3) + ",";
  json += "\"gx_F1\":" + String(gx_F1_print, 2) + ",";
  json += "\"gy_F1\":" + String(gy_F1_print, 2) + ",";
  json += "\"gz_F1\":" + String(gz_F1_print, 2) ;
  json += "}";
  server.send(200, "application/json", json);
}

void handleStatus_F1() {
  String json = "{";
  json += "\"desired_speed_m1_F\":" + String(desired_speed_motor1) + ",";
  json += "\"actual_speed_m1_F\":" + String(calc_vel_motor(currentVelocity1)) + ",";
  json += "\"desired_speed_m2_F\":" + String(desired_speed_motor2) + ",";
  json += "\"actual_speed_m2_F\":" + String(calc_vel_motor(currentVelocity2)) + ",";
