...
const float GY_F2RO_SCALE = 131.0;     // ±250°/s

// Raw sensor variables
int16_t ax_F2, ay_F2, az_F2, gx_F2, gy_F2, gz_F2;

// Offset variables
float ax_F2_offset = 0, ay_F2_offset = 0, az_F2_offset = 0;
float gx_F2_offset = 0, gy_F2_offset = 0, gz_F2_offset = 0;

//temporary for serial printing
float ax_F2_print=0.0,ay_F2_print=0.0,az_F2_print=0.0,gx_F2_print=0.0,gy_F2_print=0.0,gz_F2_print=0.0;

// Kalman filter states for each gy_F2ro ax_F2is
struct KalmanGy_F2ro {
  float rate = 0.0;
  float bias = 0.0;
  float P[2][2] = {{1, 0}, {0, 1}};
} kalX, kalY, kalZ;
...
//desired speed
float desired_linear_speed_F2 = 0.0;
float desired_angular_speed_F2 = 0.0;
...
void setup() {
  ...
  server.on("/v_desired_F2",HTTP_GET, handle_v_desired_F2);
  server.on("/status_F2",HTTP_GET, handleStatus_F2);
  server.on("/set_pid",HTTP_GET, handlePIDParams);
  server.on("/set_pwm_F2",HTTP_GET, handlePwm_F2);
  server.on("/imu_F2",HTTP_GET,handle_imu_F2); 
  server.begin();
}

void loop() {

  mpu.getMotion6(&ax_F2, &ay_F2, &az_F2, &gx_F2, &gy_F2, &gz_F2);
  ...
  float ax_F2_corr = (ax_F2 - ax_F2_offset) / ACCEL_SCALE;
  float ay_F2_corr = (ay_F2 - ay_F2_offset) / ACCEL_SCALE;
  float az_F2_corr = (az_F2 - az_F2_offset) / ACCEL_SCALE;

  // Raw gy_F2ro rates in °/s after calibration
  float gx_F2_corr = (gx_F2 - gx_F2_offset) / GY_F2RO_SCALE;
  float gy_F2_corr = (gy_F2 - gy_F2_offset) / GY_F2RO_SCALE;
  float gz_F2_corr = (gz_F2 - gz_F2_offset) / GY_F2RO_SCALE;

  // Kalman filtered gy_F2ro
  float filt_gx_F2 = kalman1D(gx_F2_corr, dt, kalX);
  float filt_gy_F2 = kalman1D(gy_F2_corr, dt, kalY);
  float filt_gz_F2 = kalman1D(gz_F2_corr, dt, kalZ);

  float temp_ax_F2 = ax_F2_corr;
  float temp_ay_F2 = ay_F2_corr;
  ax_F2_corr = -temp_ay_F2;
  ay_F2_corr = temp_ax_F2;

  float temp_gx_F2 = filt_gx_F2;
  float temp_gy_F2 = filt_gy_F2;
  filt_gx_F2 = -temp_gy_F2;
  filt_gy_F2 = temp_gx_F2;

  ax_F2_print=ax_F2_corr;
  ay_F2_print=ay_F2_corr;
  az_F2_print=az_F2_corr;
  gx_F2_print=filt_gx_F2;
  gy_F2_print=filt_gy_F2;
  gz_F2_print=filt_gz_F2;
...
void pid_control_motor() {
  ...
  if (now - last_pid_time >= 100) {  // Run at 10 Hz note that in calculating integral terms we use *0.1
    ...
    // PID calculations
    pid_integral_m1 += error_m1*0.1;
    pid_integral_m2 += error_m2*0.1;
    pid_integral_integral_m1+=pid_integral_m1*0.1;
    pid_integral_integral_m2+=pid_integral_m2*0.1;
    ...
    // Clamp output between 0 and 0.75 (max_F2 duty)
...
void calibrateMPU() {
  long ax_F2_sum = 0, ay_F2_sum = 0, az_F2_sum = 0;
  long gx_F2_sum = 0, gy_F2_sum = 0, gz_F2_sum = 0;

  for (int i = 0; i < 20000; i++) {
    mpu.getMotion6(&ax_F2, &ay_F2, &az_F2, &gx_F2, &gy_F2, &gz_F2);
    ax_F2_sum += ax_F2; ay_F2_sum += ay_F2; az_F2_sum += az_F2;
    gx_F2_sum += gx_F2; gy_F2_sum += gy_F2; gz_F2_sum += gz_F2;
    delay(5);
  }

  ax_F2_offset = ax_F2_sum / 20000.0;
  ay_F2_offset = ay_F2_sum / 20000.0;
  az_F2_offset = (az_F2_sum / 20000.0) - ACCEL_SCALE; // Should be 1g
  gx_F2_offset = gx_F2_sum / 20000.0;
  gy_F2_offset = gy_F2_sum / 20000.0;
  gz_F2_offset = gz_F2_sum / 20000.0;
}

float kalman1D(float newRate, float dt, KalmanGy_F2ro &kal) {
  // Predict
...
//server handlers
void handle_v_desired_F2(){
  if (server.hasArg("desired_linear_velocity_F2")){
    desired_linear_speed_F2=server.arg("desired_linear_velocity_F2").toFloat();
  }
  if (server.hasArg("desired_angular_velocity_F2")){
    desired_angular_speed_F2=server.arg("desired_angular_velocity_F2").toFloat();
  }
  desired_speed_motor1=desired_linear_speed_F2+(desired_angular_speed_F2*wheel_base)/2.0;
  desired_speed_motor2=desired_linear_speed_F2-(desired_angular_speed_F2*wheel_base)/2.0;
...
void handlePwm_F2() {
  if (server.hasArg("initial_pr_F2")) {initial_pr = server.arg("initial_pr_F2").toFloat();}
  if (server.hasArg("initial_pl_F2")) {initial_pl = server.arg("initial_pl_F2").toFloat();}
  server.send(200, "text/plain", "PID parameters updated");
}

void handle_imu_F2(){
  String json = "{";
  json += "\"ax_F2\":" + String(ax_F2_print, 3) + ",";
  json += "\"ay_F2\":" + String(ay_F2_print, 3) + ",";
  json += "\"az_F2\":" + String(az_F2_print, 3) + ",";
  json += "\"gx_F2\":" + String(gx_F2_print, 2) + ",";
  json += "\"gy_F2\":" + String(gy_F2_print, 2) + ",";
  json += "\"gz_F2\":" + String(gz_F2_print, 2) ;
  json += "}";
  server.send(200, "application/json", json);
}


void handleStatus_F2() {
  String json = "{";
  json += "\"desired_speed_m1_F2\":" + String(desired_speed_motor1) + ",";
  json += "\"actual_speed_m1_F2\":" + String(calc_vel_motor(currentVelocity1)) + ",";
  json += "\"desired_speed_m2_F2\":" + String(desired_speed_motor2) + ",";
  json += "\"actual_speed_m2_F2\":" + String(calc_vel_motor(currentVelocity2)) + ",";
